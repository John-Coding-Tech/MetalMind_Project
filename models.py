...
class SupplierEmail(Base):
    """Per-supplier email log — outbound RFQs the user sent and inbound replies
    they pasted back. Records both AI-drafted and manually-written messages.

    Design principle (locked in docs/email-log-feature.md):
    Event time (sent_at / received_at) is separate from processing time
    (created_at). Event time is user-editable so logging an email after a
    paste delay doesn't pollute future supplier-responsiveness analytics.
    """
    ...
    # 'outbound' (user sent / AI-drafted) or 'inbound' (supplier reply pasted in)
    direction   = Column(String(20), nullable=False, index=True)

    # True when the body was generated by the chat AI (vs hand-typed).
    ai_generated = Column(Boolean, default=False, nullable=False)

    # Processing time — when the row hit the DB. Auto, immutable.
    created_at  = Column(DateTime, default=lambda: datetime.now(timezone.utc),
                         nullable=False)
    # Event time — when the email actually went out / came in.
    # Both nullable; only one is meaningful per row depending on direction.
    # User-editable in the UI for inbound (so old replies can be timestamped
    # accurately), null until user marks an outbound draft as actually sent.
    sent_at     = Column(DateTime, nullable=True)
    received_at = Column(DateTime, nullable=True)

    # Light grouping for v2 threading UI. v1 leaves this null and renders flat.
    thread_id   = Column(String(50), nullable=True, index=True)

    supplier = relationship("SavedSupplier", back_populates="emails")
